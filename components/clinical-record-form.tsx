...
export function ClinicalRecordForm({
  ...
  const [formData, setFormData] = useState({
    ...
    visual_acuity: {
    od_sin_correccion_lejana: record?.od_far_without_correction || "",
    od_con_correccion_lejana: record?.od_far_with_correction || "",
    od_sin_correccion_cercana: record?.od_near_without_correction || "",
    od_con_correccion_cercana: record?.od_near_with_correction || "",
    oi_sin_correccion_lejana: record?.oi_far_without_correction || "",
    oi_con_correccion_lejana: record?.oi_far_with_correction || "",
    oi_sin_correccion_cercana: record?.oi_near_without_correction || "",
    oi_con_correccion_cercana: record?.oi_near_with_correction || "",
    ao_sin_correccion_lejana: record?.ao_far_without_correction || "",
    ao_con_correccion_lejana: record?.ao_far_with_correction || "",
    ao_sin_correccion_cercana: record?.ao_near_without_correction || "",
    ao_con_correccion_cercana: record?.ao_near_with_correction || "",
  },
    ph_od: record?.ph_od || "",
    ph_oi: record?.ph_oi || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!formData.patient_id) { setError("Debes seleccionar un paciente"); setActiveTab(0); setLoading(false); return }
  ...
  // Navega al tab de forma segura, sin propagar eventos al DOM
  const goToTab = (e: React.MouseEvent, index: number) => {
    e.preventDefault()
    e.stopPropagation()
    setActiveTab(index)
  }
  ...
  return (
    <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="space-y-2">
                ...
                <Input type="date" value={formData.exam_date} onChange={(e) => updateField("exam_date", e.target.value)} disabled={loading} />
              </div>
              <div className="space-y-2">
            ...
            <CardContent>
              <div className="flex items-center gap-4">
                <div
                  ...
                  className="flex items-center gap-2 cursor-pointer border rounded-md px-4 py-2 text-sm hover:bg-accent transition-colors"
                >
      ...
      {/* Navegación — sin ningún <form>, el submit llama handleSubmit directamente */}
        ...
        <div className="flex gap-2">
          {activeTab > 0 && (
            <Button
              type="button"
              variant="outline"
              onClick={(e) => goToTab(e, activeTab - 1)}
              disabled={loading}
            >
